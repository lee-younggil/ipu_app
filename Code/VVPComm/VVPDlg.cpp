...
CVVPDlg::CVVPDlg(QWidget *parent) : QWidget(parent)
{
    CVVPComm* comm = CMediator::getInstance()->getVvpComm();
    setVvpComm(comm); // Assuming setVvpComm stores it in m_vvpComm
    CreateLayout();

    if (m_vvpComm) {
        // Use Qt::QueuedConnection for thread safety as signals might come from different threads
        connect(m_vvpComm, &CVVPComm::packetSent, this, [this](const QByteArray& data){
            logPacket(data, "T");
        }, Qt::QueuedConnection);
        connect(m_vvpComm, &CVVPComm::packetRecv, this, [this](const QByteArray& data){
            logPacket(data, "R");
        }, Qt::QueuedConnection);

    } else {
        qDebug() << "CVVPDlg: VVPComm instance is null, cannot connect signals.";
    }
}
...
void CVVPDlg::CreateLayout()
{
    // Create tab page for VVP communication
    ...
    m_func2Button = new QPushButton(tr("SendAck"));
    if (!m_func2Button) {
        qDebug() << "Failed to create button";
        return;
    }
    ...
    connect(m_func1Button, &QPushButton::clicked, this, &CVVPDlg::Func1);
    connect(m_func2Button, &QPushButton::clicked, this, &CVVPDlg::SendAck);
    connect(m_sendButton, &QPushButton::clicked, this, &CVVPDlg::LineCmd);
...
void CVVPDlg::SendAck()
{
    if (m_vvpComm) {
        //m_vvpComm->SendAck(DEVICE_CODE_MCU, m_vvpComm->GetSeqNo(DEVICE_CODE_MCU));
        m_vvpComm->SendAck(m_vvpComm->GetSeqNo(0x01));
    }
}

void CVVPDlg::LineCmd()
{
    if (m_vvpComm) {
        //m_vvpComm->LineCmd();
    }
...
void CVVPDlg::logPacket(const QByteArray& data, const QString& prefix)
{
    //qDebug() << ">>> CVVPDlg::logPacket start" << prefix << " <<<";
    if (!m_textEdit) {
    ...
    QString logEntry = QString("%1 [%2] %3").arg(timestamp).arg(prefix).arg(hexString);
